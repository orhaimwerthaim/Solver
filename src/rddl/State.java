...
public class State {
	...
	public boolean checkStateActionConstraints(ArrayList<PVAR_INST_DEF> actions)
			throws EvalException {
		return checkStateActionConstraints(actions, true);
	}
	public boolean checkStateActionConstraints(ArrayList<PVAR_INST_DEF> actions, boolean withException)
		throws EvalException {
		...
		if (non_def > _nMaxNondefActions)
			if(withException)throw new EvalException("Number of non-default actions (" + non_def +
		...
		for (BOOL_EXPR constraint : _alActionPreconditions) {
			...
			try {
				if (! (Boolean)constraint.sample(subs, this, null) )
					if(withException) throw new EvalException("Violated state invariant or action precondition constraint: " + constraint + "\n**in state**\n" + this);
					else return false;
